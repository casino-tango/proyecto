...
export async function buscarNuc(req, res) {
  ...
  try {
    const buscarNuc1 = await maquina.findOne({
      where: {
        Numero_unico_coljuegos: Numero_unico_coljuegos
      },
      include: fotografia
    });
    res.json(buscarNuc1);
...
export async function editar_maquina1(req, res) {
  ...
  try {
    ...
    const maquinaAntesDeEditar = await maquina.findOne({
      where: {
        Numero_serial
      }
    });
    ...
    for (const key in nuevosValores) {
      if (valoresOriginales[key] !== nuevosValores[key]) {
        camposEditados.push({
          campo: key,
          valorAntiguo: valoresOriginales[key],
          valorNuevo: nuevosValores[key]
        });
      }
    }
    ...
    if (!Array.isArray(camposEditados)) {
      ...
      return res.status(500).json({
        success: false,
        message: "Ha ocurrido un error al editar la máquina"
      });
    }
