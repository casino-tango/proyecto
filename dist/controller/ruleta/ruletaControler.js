...
export async function mirar(req, res) {
  try {
    ...
    if (usuario.rol === 'Admin') {
      ...
      // Utilizando el nombre correcto del modelo
      {
        attributes: {
          exclude: ['certificado_de_importacion_ruleta', 'Factura_compra_ruleta', 'serial_modelo_ruleta', 'fotografia_de_respaldo', 'fotografia_de_placa', 'fotografia_de_billetero']
        }
      });
      ...
      if (!ubicacion_del_elemento) {
        return res.status(400).json({
          error: 'La ubicación del elemento es requerida'
        });
      }
      datos = await ruleta.findAll({
        where: {
...
export async function mirar_ruleta_ubiccacion(req, res) {
  ...
  try {
    const buscarid = await ruleta.findAll({
      ...
      attributes: {
        exclude: ['certificado_de_importacion_ruleta', 'fotografia_de_ruleta', 'Factura_compra_ruleta', 'serial_modelo_ruleta', 'fotografia_de_respaldo', 'fotografia_de_placa', 'fotografia_de_billetero']
      }
      // include: fotografia
    });
    res.json(buscarid);
