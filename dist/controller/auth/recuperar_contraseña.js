...
export const recuperar = async (req, res) => {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      // Cambia esto por tu dirección de correo electrónico desde donde se envia el manseje
  ...
  const mensaje = {
    ...
    to: email,
    // Email del destinatario obtenido desde la solicitud
  ...
  transporter.sendMail(mensaje, async (error, info) => {
    if (verificacion) {
      ...
      codigo.codigoRecuperar = codigo_aleatorio, await codigo.save();
      res.status(200).json({
...
export const ActualizarContrasena = async (req, res) => {
  try {
    ...
    await user.save();
    res.status(200).json({
      message: 'Se ha actualizado tu contraseña correctamente',
      user
    });
  } catch (error) {
    console.log(error);
